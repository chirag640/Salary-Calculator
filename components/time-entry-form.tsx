...
interface TimeEntryFormProps {
  selectedDate: string;
  onSubmit: (
    entry: Omit<TimeEntry, "_id" | "createdAt" | "updatedAt" | "userId">
  ) => void;
...
export function TimeEntryForm({
  ...
  const [leaveType, setLeaveType] = useState<LeaveType>(
    initialData?.leave?.leaveType || "Sick"
  );
  const [leaveReason, setLeaveReason] = useState(
    initialData?.leave?.leaveReason || ""
  );
  const [isHolidayWork, setIsHolidayWork] = useState<boolean>(
    initialData?.isHolidayWork || false
  );
  const [holidayCategory, setHolidayCategory] = useState<
    "sunday" | "saturday" | "other"
  >(initialData?.holidayCategory || "sunday");
  ...
  const calculation = (() => {
    ...
    if (isHolidayWork) {
      if (isHolidayExtra && timeIn && timeOut) {
        const extraCalc = calculateTimeWorked(
          timeIn,
          timeOut,
          breakMinutes,
          hourlyRate
        ...
        const earnings =
          hourlyRate > 0 ? Math.round(total * hourlyRate * 100) / 100 : 0;
    ...
    if (timeIn && timeOut) {
      ...
      const calc = calculateTimeWorked(
        timeIn,
        timeOut,
        breakMinutes,
        hourlyRate
    ...
    if (manualHours > 0) {
      ...
      const earnings =
        hourlyRate > 0 ? Math.round(h * hourlyRate * 100) / 100 : 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLeave) {
      ...
      if (isHolidayWork) {
        ...
        if (isHolidayExtra && !(timeIn && timeOut)) {
          alert(
            "Provide a time range for extra holiday hours or uncheck 'Add extra hours'."
          );
          return;
        }
      } else {
        if (!(timeIn && timeOut) && manualHours <= 0) {
          alert(
            "Please fill in all required fields (provide a time range or manual hours)"
          );
          return;
        }
        ...
        if (hourlyRate <= 0) {
          const ok =
            typeof window !== "undefined"
              ? window.confirm(
                  "Hourly rate is not set for the selected date. Total earnings will be recorded as $0. Do you want to continue?"
                )
              : true;
          if (!ok) return;
        }
        ...
        <CardHeader className="p-4 md:p-6">
          ...
          {!isEditing && (
            <p className="text-sm text-muted-foreground mt-2">
              Manually log your work hours with specific times. Use Quick Start
              Timer above for real-time tracking.
            </p>
          )}
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                  ...
                  {isHolidayWork && (
                    <div className="ml-6 space-y-2">
                      <Select
                        ...
                        onValueChange={(v) => setHolidayCategory(v as any)}
                      >
            ...
            {isLeave ? (
              <>
                ...
                <div>
                  <Label htmlFor="leaveReason" className="text-base">
                    Reason (Optional)
                  </Label>
                  <Textarea
                    id="leaveReason"
              ...
              <>
                ...
                {/* Hourly Rate Display - Hidden for privacy, rate is automatically fetched */}

                {/* Work Description */}
                <div>
                  <Label htmlFor="workDescription" className="text-base">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 p-4 md:p-6 rounded-lg border-2 border-blue-200 dark:border-blue-800">
                  ...
                  {calculation.totalHours === 0 && (
                    <p className="text-xs text-muted-foreground mt-2">
                      No hours computed yet — ensure start and end times are
                      filled or enter manual hours.
                    </p>
                  )}
                </div>
              </>
