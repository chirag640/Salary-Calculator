...
export function useTimer({
  ...
  // Timer action helper
  const performTimerAction = useCallback(
    async (action: string) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        // For start action, initialize elapsed seconds to 0
        if (action === "start") {
          setElapsedSeconds(0);
        }
  ...
  // Fetch timer state only once on mount if autoFetch is true
  useEffect(() => {
  ...
  // Removed visibility change sync to reduce API calls
  // Timer state is managed locally with tick interval
