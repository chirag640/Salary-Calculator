...
export async function POST(request: NextRequest) {
  ...
  const ip = fwd ? fwd.split(",")[0].trim() : request.headers.get("x-real-ip") || "unknown"
    ...
    const genericResponse = NextResponse.json({ message: "If that account exists, a reset email has been sent." })
    ...
    await db.collection("users").updateOne(
      { _id: user._id },
      { $set: { passwordResetToken: hashed, passwordResetExpires: expires, updatedAt: new Date() } }
    )
