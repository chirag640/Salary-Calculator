  ...
  try {
    ...
    if (!rateLimitResult.ok) {
      return NextResponse.json(
        {
          error: "Too many unlink attempts. Please try again later.",
    ...
    if (!fullUser.password) {
      return NextResponse.json(
        {
          error:
            "Cannot unlink Google account. Please set a password first to ensure you can still access your account.",
        },
        { status: 400 },
      );
    }
    ...
    if (!password) {
      return NextResponse.json(
        { error: "Password is required to unlink account" },
        { status: 400 },
      );
    }
    ...
    if (!fullUser.googleId) {
      return NextResponse.json(
        { error: "No Google account is linked to this account" },
        { status: 400 },
      );
    }
