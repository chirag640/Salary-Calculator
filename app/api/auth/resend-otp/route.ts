...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailRateLimit = rateLimit(emailRateLimitKey, { windowMs: OTP_CONFIG.RESEND_COOLDOWN_SECONDS * 1000, max: 1 })
    ...
    const ipRateLimit = rateLimit(ipRateLimitKey, { windowMs: 24 * 60 * 60 * 1000, max: OTP_CONFIG.MAX_RESENDS_PER_DAY })
    ...
    if (resendCount >= 3) {
      return NextResponse.json({ error: "Maximum resend attempts reached. Please start over." }, { status: 400 })
    }
