...
export async function POST(request: NextRequest) {
  try {
    ...
    // Authenticate user from auth-token cookie
    ...
    if (!rl.ok) {
      return NextResponse.json({ message: "Too many attempts, try later", retryAfter: rl.retryAfter }, { status: 429 })
    }
    ...
    if (!ok) {
      ...
      try {
        await db.collection("users").updateOne({ _id: user._id }, { $inc: { pinFailedAttempts: 1 } })
      } catch (e) {
        // ignore
      }
    ...
    try {
      await db.collection("users").updateOne({ _id: user._id }, { $set: { pinFailedAttempts: 0 } })
    } catch (e) {
      // ignore
    }
