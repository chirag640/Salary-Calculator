...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (!linkData.timestamp || Date.now() - linkData.timestamp > 10 * 60 * 1000) {
        return NextResponse.json({ error: "Link token has expired. Please sign in with Google again." }, { status: 400 })
      }
    } catch {
    ...
    const rateLimitKey = buildRateLimitKey(ip, `link-account:${sanitizedEmail}`)
    const rateLimitResult = rateLimit(rateLimitKey, { windowMs: 15 * 60 * 1000, max: 5 })
    
    if (!rateLimitResult.ok) {
      return NextResponse.json(
        { error: "Too many linking attempts. Please try again later.", retryAfter: rateLimitResult.retryAfter },
        { status: 429 }
      )
    }
    ...
    if (method === "password") {
      ...
      if (!user.password) {
        return NextResponse.json({ error: "This account does not have a password. Please use OTP verification." }, { status: 400 })
      }
      ...
      if (!otp) {
        ...
        return NextResponse.json({ error: "Use /api/auth/verify-link-otp to verify OTP" }, { status: 400 })
      }
    } else {
