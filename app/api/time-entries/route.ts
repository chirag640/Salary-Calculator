...
import {
  getEffectiveHourlyRateForDate,
  computeEarningsWithOvertime,
} from "@/lib/salary";
  ...
  try {
    ...
    if (limit > 0 || cursor) {
      ...
      const docs = await collection
        ...
        .toArray();
      const nextCursor =
        docs.length === pageSize ? docs[docs.length - 1]._id.toString() : null;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimit(buildRateLimitKey(ip, "time-entry-create"), {
      windowMs: 30_000,
      max: 10,
    });
    if (!rl.ok) {
    ...
    try {
      if (request && request.cookies) {
        ...
        const getFn: any =
          (request.cookies.get && request.cookies.get.bind(request.cookies)) ||
          undefined;
        if (typeof getFn === "function") {
          cookiePresent = !!getFn("csrf-token")?.value;
        } else if (
          typeof request.cookies === "object" &&
          typeof (request.cookies as any).get === "function"
        ) {
    ...
    if (!leave?.isLeave) {
      if (isHolidayWork) {
        ...
        if (timeIn && timeOut) {
          totalHours = calculateTimeWorked(
            timeIn,
            timeOut,
            breakMinutes,
            hourlyRate
    ...
    const doc: TimeEntry = {
      ...
      holidayCategory: isHolidayWork
        ? holidayCategory || deriveHolidayCategory(date)
        : undefined,
