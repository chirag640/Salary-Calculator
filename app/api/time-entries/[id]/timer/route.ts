...
function calculateElapsedSeconds(timer: TimerState): number {
  ...
  if (timer.isRunning) {
    ...
    const lastResumeTime =
      timer.resumedAt && timer.resumedAt.length > 0
...
export async function POST(
  ...
  try {
    ...
    const rl = rateLimit(buildRateLimitKey(ip, "timer-action"), {
      windowMs: 10_000,
      max: 30,
    });
    if (!rl.ok) {
    ...
    if (
      !action ||
      !["start", "pause", "resume", "stop", "heartbeat"].includes(action)
    ) {
    ...
    switch (action) {
      ...
      case "stop":
        ...
        // Mark timer as stopped and clear running state
