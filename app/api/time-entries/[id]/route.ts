...
export async function PUT(
  ...
  try {
    ...
    const rl = rateLimit(buildRateLimitKey(ip, "time-entry-update"), {
      windowMs: 30_000,
      max: 20,
    });
    if (!rl.ok) {
...
export async function DELETE(
  ...
  try {
    ...
    const ip =
      request.headers.get("x-forwarded-for")?.split(",")[0] || "unknown";
    const rl = rateLimit(buildRateLimitKey(ip, "time-entry-delete"), {
      windowMs: 30_000,
      max: 30,
    });
    if (!rl.ok) {
...
export async function POST(
  ...
  try {
    ...
    const ip =
      request.headers.get("x-forwarded-for")?.split(",")[0] || "unknown";
    const rl = rateLimit(buildRateLimitKey(ip, "time-entry-restore"), {
      windowMs: 30_000,
      max: 30,
    });
    if (!rl.ok) {
