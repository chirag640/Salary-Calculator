  ...
  try {
    ...
    if (!revealValid || revealValid.userId !== userId) {
      return NextResponse.json(
        {
          error:
            "Reveal required to export monetary data. Please reveal sensitive values first.",
        },
        { status: 403 },
      );
    }
    ...
    const rl = rateLimit(buildRateLimitKey(ip, "export"), {
      windowMs: 60_000,
      max: 5,
    });
    if (!rl.ok)
    ...
    const exportRows = entries.map((entry) => ({
      ...
      "Holiday Category": entry.isHolidayWork
        ? entry.holidayCategory || ""
        : "",
    }));
